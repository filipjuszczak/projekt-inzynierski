...
import { authUser, validateBuySession } from "@/lib/auth/helpers";
...
export async function createCheckoutSession({
  ...
  try {
    ...
    cancelUrlParams.set("buySessionId", buySession.id);
    for (const seat of selectedSeats) {
      cancelUrlParams.append("seat", `${seat.rowNumber}-${seat.seatNumber}`);
    }

    const stripeSession = await stripe.checkout.sessions.create({
      success_url: `${process.env.NEXT_PUBLIC_BASE_URL}/twoja-rezerwacja?id=${createdOrder.id}`,
